...
/**
 * Block non-members from completing DiscourseConnect SSO login.
 */
function matonizz_gate_discourse_sso($user_id, $user)
{
	...
	foreach ($member_group_map as $item) {
		$membership = wc_memberships_get_user_membership($user_id, $item->plan_id);
		if ($membership && in_array($membership->status, ACTIVE_STATUSES, true)) {
	...
	if (! $is_active_member) {
		wp_safe_redirect(home_url('/membership-required/'));
		exit;
	}
}
